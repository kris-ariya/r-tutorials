...
```
# Load Data
Save a data file and an R script in the same directory. Set working directory to Source File Location. 
...
For each *X*, its standardized score, *z*, can be calculated as
$$z = \frac{X-\bar{X}}{s}$$

### Manual method
You can calculate *z*-score step by step.
First, the top part of the equation $X-\bar{X}$ is called **mean centering**. That is, the value X is subtracted by the mean of X, `X - mean(X)`.  
...
The first argument of `by` is an input data, `knowledge`. The second argument is a grouping factor, `inno`. The third argument is a function to apply, `describe`.
